...
export function PerspectivePreview({ mode, councilSize, agentCount, onSettings }: { mode: DebateMode; councilSize: CouncilSize; agentCount: number; onSettings: () => void }) {
  ...
  return <section className="perspective-canvas" aria-label="What to expect">
    ...
    <div className="canvas-status"><span>{modeGuide[mode].name}</span><span>{mode === "consensus" ? `${agentCount} independent perspectives` : mode === "advisory_panel" ? "4 advisors + a chair" : `${councilSize === "duo" ? "2" : "4"} debaters + a judge`}</span><span>How it works</span></div>
