...
SERVICE="${GCP_POLYVISE_SERVICE:-polyvise-ai-web}"
...
gcloud run deploy "$SERVICE" \
  ...
  --timeout 300 \
  --set-env-vars "^|^NEXT_PUBLIC_SITE_URL=$SITE_URL|POLYVISE_REPOSITORY=${POLYVISE_REPOSITORY:-firestore}|FIRESTORE_PROJECT_ID=${FIRESTORE_PROJECT_ID:-$PROJECT_ID}|POLYVISE_ENABLE_MOCK_LLM=${POLYVISE_ENABLE_MOCK_LLM:-false}|POLYVISE_EVIDENCE_PROVIDER=${POLYVISE_EVIDENCE_PROVIDER:-tavily}" \
